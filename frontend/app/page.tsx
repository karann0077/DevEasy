...
export default function IngestPage() {
  ...
  const handleIngest = async () => {
    ...
    // Build backend URL using env var: NEXT_PUBLIC_API_BASE_URL or NEXT_PUBLIC_API_URL
    const apiBase =
    ...
    const targetUrl = apiBase ? `${apiBase}/api/ingest` : `/api/ingest`;

    // Log the actual request target so you can debug quickly in UI
    setLogs((prev) => [...prev, `➡️ Posting to ${targetUrl}`]);
    ...
    try {
      ...
      if (!resp.ok) {
        ...
        const errLogs: string[] = err ? extractLogs(err) : [`❌ Error: ${resp.statusText}`];
        setLogs(errLogs);
        setStatus("error");
        return;
      }
          ...
          <div className="mb-6 flex items-center gap-4 p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/30">
            ...
            <div>
              ...
              <div className="text-slate-400 text-sm mt-0.5">
                <span className="text-white font-semibold">{chunksIngested.toLocaleString()}</span> code chunks vectorized and stored in Pinecone. Ready for RAG queries.
              </div>
            </div>
        ...
        {status === "error" && (
          <div className="mb-6 flex items-center gap-4 p-4 rounded-xl bg-red-500/10 border border-red-500/30">
            ...
            <div>
              ...
              <div className="text-slate-400 text-sm mt-0.5">Check the logs above for details. Ensure your API keys are configured.</div>
            </div>
          </div>
        )}
